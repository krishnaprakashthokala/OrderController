...
public class ReviewController {
	...
	public String create(@PathVariable Long productId, Model model) {
		...
		logger.info("get  Review Controller  create: " + url);

		return String.format("frontend/fragments/product/review::create(url='%s')", url);
	}

	@PostMapping("/create")
	public String create(@PathVariable Long productId, @ModelAttribute(ATTRIBUTE_NAME) @Valid Review review,
			BindingResult bindingResult, RedirectAttributes model,
			// SessionStatus lets you clear your SessionAttributes
			SessionStatus sessionStatus, @CurrentUserAttached User currentUser) {
